...
int UDP_Open(int port) {
    ...
    if ((sd = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
        // Se socket() fallisce, restituiamo -1 per indicare errore al chiamante
        return -1;
    ...
    if (bind(sd, (struct sockaddr *) &myaddr, sizeof(myaddr)) == -1) {
        // In caso di errore nel bind, chiudiamo la socket per pulizia
        close(sd);
        // Restituiamo -1 per segnalare che qualcosa è andato storto
        return -1;
    }
