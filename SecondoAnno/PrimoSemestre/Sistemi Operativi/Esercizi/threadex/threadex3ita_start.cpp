...
pthread_cond_t cond1 = PTHREAD_COND_INITIALIZER; //mi serve per la comunicazione tra i thread
// Dichiara le funzioni dei thread: functionCount1 e functionCount2
void * functionCount1(void*);
void* functionCount2(void*);

int count = (intptr_t)0;
int go = 0;
...
int main()
{
    ...
    int check1= pthread_create(&thread1,NULL,functionCount1,(void*)(intptr_t)count);
    int check2= pthread_create(&thread2,NULL,functionCount2,(void*)(intptr_t)count);

    // Attende che i thread siano completati prima che main continui

    pthread_join(thread1,NULL) ; 
    pthread_join(thread2,NULL ) ; //si potrebbe mettere un valore (dove ora c'è NULL ) che rappresenterebbe il valore di ritorno della funzione
