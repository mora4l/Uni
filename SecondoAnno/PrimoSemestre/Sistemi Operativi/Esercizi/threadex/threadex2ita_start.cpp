...
pthread_mutex_t PrimoMutexCreato = PTHREAD_MUTEX_INITIALIZER;
...
int main()
{
    ...
    int errore1 = pthread_create(&thread1,NULL,incrementCounter,(void *)(intptr_t) counter);
    int errore2 = pthread_create(&thread2,NULL,incrementCounter,(void*)(intptr_t)counter);

    if(errore1 != 0 ){
        printf("creazione del thread1 fallita\n") ; 
    }
    if(errore2 != 0 ){
        printf("creazione del thread2 fallita\n") ; 
    }
...
void* incrementCounter(void * )
{
    for (int i=0; i < 100; i++)
    {
        ...
        pthread_mutex_lock(&PrimoMutexCreato);
	// aggiorno temp e lo incremento di uno
        ...
        pthread_mutex_unlock(&PrimoMutexCreato) ; 
    }
    return NULL;
}  
